...
from mediatool.metadata import get_metadata_args, read_metadata
...
def split_video(input_file: Path, change_artist=False):
    ...
    while info := Prompt.ask(f'Enter time point and title of {input_file}').strip():
        ...
        while not meta.get('artist'):
            meta['artist'] = Prompt.ask(
                f'Enter the artist of {input_file}').strip()
        for k in ['genre', 'creation_time']:
            if meta.get(k):
                continue
            if not (v := Prompt.ask(f'Enter {k} of {input_file}').strip()):
                continue
            if k == 'creation_time':
                v = pendulum.from_format(
